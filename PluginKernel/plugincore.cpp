...
bool PluginCore::initPluginParameters()
{
	...
	piParam = new PluginParameter(controlID::intensity_Pct, "Intensity", "%", controlVariableType::kDouble, 0.000000, 100.000000, 50.000000, taper::kLinearTaper);
	piParam->setParameterSmoothing(true);
	piParam->setSmoothingTimeMsec(20.00);
...
bool PluginCore::reset(ResetInfo& resetInfo)
{
	...
	// --- reset components
	leftPhaseShifter.reset(resetInfo.sampleRate);
	rightPhaseShifter.reset(resetInfo.sampleRate);
...
{
	PhaseShifterParameters params = leftPhaseShifter.getParameters();
...
bool PluginCore::processAudioFrame(ProcessFrameInfo& processFrameInfo)
{
	...
	double ynL = leftPhaseShifter.processAudioSample(xnL);
       ...
       processFrameInfo.channelIOConfig.outputChannelFormat == kCFStereo)
    {
		...
		double ynR = rightPhaseShifter.processAudioSample(xnR);
...
bool PluginCore::initPluginPresets()
{
	...
	setPresetParameter(preset->presetParameters, controlID::intensity_Pct, 50.000000);
	addPreset(preset);
